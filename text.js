...
// dino 
function draw() {
    dinoContext.drawImage(land, 0, yPos + 70);
    ...
    for (let i = 0; i < barrier.length; i++) {
        if (barrier[i].x < -cactus.width) {
            barrier.shift();
        } else {
            dinoContext.drawImage(cactus, barrier[i].x, barrier[i].y);
            barrier[i].x -= 2;

            if (barrier[i].x == 90) {
                barrier.push({
                    x: dinoCanvas.width,
                    y: Math.floor(Math.random() * cactus.height),
                 });
            }
        }
        ...
        if (
            ...
            (yPos <= barrier[i].y + cactus.height)) {
            reload();
        }
    }
}
